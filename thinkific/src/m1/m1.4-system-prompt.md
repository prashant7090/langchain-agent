...
<p style="font-size:13px;color:#946200;font-family:'IBM Plex Mono',monospace;margin:0 0 10px;">Note: as of the deepagents 0.7 update, <code>create_deep_agent</code>'s built-in base system prompt is empty by default (previously it always added its own behavior guidance underneath yours). The page content below reflects this; the video walkthrough has not been re-recorded yet.</p>
...
In Deep Agents, you pass your own instructions with the `system_prompt` argument. As of deepagents 0.7, `create_deep_agent`'s authored base prompt is empty by default, so whatever you pass as `system_prompt` **is** the system prompt sent to the model: nothing else is mixed in underneath it automatically.
...
<tbody>
<tr><td><strong>Your instructions</strong></td><td>The <code>system_prompt</code> argument you pass to <code>create_deep_agent</code>. By default, this <em>is</em> the full system prompt.</td></tr>
<tr><td><strong>Built-in guidance (opt-in)</strong></td><td>Empty by default as of deepagents 0.7. Restore the SDK's old default guidance by passing <code>system_prompt=BASE_AGENT_PROMPT</code> (deprecated; scheduled for removal in deepagents 0.9).</td></tr>
</tbody>
</table>

If you don't pass `system_prompt` at all, the agent gets no system prompt by default, which is exactly why it's worth setting one.
...
<div style="margin:1.5rem 0;padding:1.25rem 1.5rem;border-radius:12px;border:1px solid #99d3ff;background-color:#e5f4ff;color:#1e4d7a;"><p>In Lab 1, you'll add a <strong>system prompt</strong> for the first time by passing a persona (pirate, cowboy, Shakespeare) and see how that single line steers the agent's voice without touching anything else.</p><p>In this example, I've given it a posh British butler persona.</p><p><a href="https://smith.langchain.com/public/d81e0469-c392-4c73-85a8-e56c9df472c9/r/019edc62-e11c-7110-8cff-020d8a1e5c99">View LangSmith trace →</a></p><details style="border:2.5px solid #000;border-radius:6px;background:#fff;margin:1rem 0;"><summary style="padding:10px 16px;cursor:pointer;font-weight:500;font-family:'IBM Plex Mono',monospace;font-size:14px;">Lesson Video Walkthrough</summary><div style="padding:12px 16px 16px;"><div class="video-container" style="max-width:750px;"><div class="video-wrapper"><iframe src="https://share.descript.com/embed/S8G4RRa5XU5" frameborder="0" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen></iframe></div></div></div></details><p>The <code>name="butler_agent"</code> argument also sets the root trace name in LangSmith, making traces easier to recognize. (Note: OpenAI's API rejects spaces in message names, so use underscores (no spaces).)</p></div>
...
**The discovery:** `system_prompt` is how you orient your agent to the task at hand and pass it essential instructions. It is free-form text: use it to define the agent's role, scope its domain, or enforce constraints. As of deepagents 0.7, whatever you pass here is the full system prompt; the SDK no longer mixes in built-in guidance underneath it by default.
...
message. As of deepagents 0.7, that message is your `system_prompt` verbatim;
nothing else is appended underneath it by default. Switch personas, re-run, and
you'll see the entire system message change to match, proof that this single
string is the whole system prompt, not just a piece of it.
...
**If you started this course before July 16, 2026:** Lab 2 is new. Pull the latest changes from the repo root to access it:
...
<details style="border:2.5px solid #000;border-radius:6px;background:#fff;margin:1rem 0;"><summary style="padding:10px 16px;cursor:pointer;font-weight:500;font-family:'IBM Plex Mono',monospace;font-size:14px;">Lab Video Walkthrough</summary><div style="padding:12px 16px 16px;"><div class="video-container" style="max-width:750px;"><div class="video-wrapper"><iframe src="https://share.descript.com/embed/115KapYsKeQ" frameborder="0" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen></iframe></div></div></div></details>

**First run — [view this run in LangSmith.](https://smith.langchain.com/public/f4beb101-176a-4705-b4d5-2147dfa45db8/r/019f67f2-6940-7b60-b416-edd2decb16bd)** Notice the system prompt and tool definitions billed at full price; there's nothing cached yet to read from.

**Second run — [view this run in LangSmith.](https://smith.langchain.com/public/09f94c0c-7e37-437e-8ea6-073bdbfeb439/r/019f67f2-87e4-7e92-935f-1e6c510f0262)** Notice the same system prompt and tools reading from cache instead of processing fresh, at a fraction of the cost.

<details style="border:2.5px solid #000;border-radius:6px;background:#E5F4FF;margin:1rem 0;"><summary style="padding:10px 16px;cursor:pointer;font-weight:500;font-family:'IBM Plex Mono',monospace;font-size:14px;">LangSmith Video Walkthrough</summary><div style="padding:12px 16px 16px;"><div class="video-container" style="max-width:750px;"><div class="video-wrapper"><iframe src="https://share.descript.com/embed/b2Jw2GfBHvk" frameborder="0" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowfullscreen></iframe></div></div></div></details>
...
<MCQ
    ...
    correctIndex={1}
    explanation="As of deepagents 0.7, the built-in base prompt is empty by default, so skipping system_prompt means the agent gets no system prompt at all, unless you explicitly restore the old default via system_prompt=BASE_AGENT_PROMPT."
/>
